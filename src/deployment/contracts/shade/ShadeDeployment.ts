...
export class ShadeDeployment {
	...
	private NUM_SNIP_20s = 3
	private LP_TOKEN_DECIMALS = 18
	private AMOUNT_TO_MINT = "10000000000000"
	private LIQUIDITY_TO_PROVIDE = "200000000"
	...
	public async deployContracts(options: ShadeDeploymentOptions): Promise<Shade> {
		...
		await this.USDCTokenSetup(factory, snip20s[0])
	...
	private async USDCTokenSetup(factory: Factory, snip20: Snip20) {
		const snip20Deployment = new Snip20Deployment(this.secretjs)
		const USDC = snip20Deployment.getCurrentDeployment()

		await factory.create_amm_pair(snip20.getContractInfo(), USDC.getContractInfo(), this.LP_TOKEN_DECIMALS);
		await snip20.mint(this.secretjs.address, `${this.AMOUNT_TO_MINT}`)
		await USDC.mint(this.secretjs.address, `${this.AMOUNT_TO_MINT}`)
	}
}
