...
import { AmmPair } from "./amm_pair/AmmPair";
...
export class Shade {
	...
	public getPath(route: Route): Hop[] {
		const hopsMap = this.getHopsMap();
		const path: Hop[] = [];

		for (let addr of route.path) {
	...
	public async getInputSnip20(route: Route): Promise<Snip20> {
	...
	public async getSnip20s(): Promise<Snip20[]> {
		const uniqueSnip20Map = await this.getSnip20sMap()
