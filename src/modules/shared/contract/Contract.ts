...
export class Contract<ExecuteMsg extends object, QueryMsg extends object, QueryAnswer extends object> {
    ...
    public static async instantiate_contract<InitMsg extends object>(
        ...
        if (!contractAddress) {
            console.error(`Error Instantiating Contract txResponse: ${JSON.stringify(tx, null, 2)}`);
     ...
     * JSON data.
     */
    public static parseTransactionData<IntoType extends object>(tx: TxResponse): IntoType {
        const transaction_data: Uint8Array[] = tx.data;
        const decoder = new TextDecoder('utf-8'); // 'utf-8' is default, can be omitted
        let transactionJsonString: string = transaction_data.map(buffer => decoder.decode(buffer)).join('');
